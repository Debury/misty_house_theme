      ...
      <div class="product-image-section">
        <div class="main-product-image">
          <?php
            ...
            if (empty($gallery)) { $gallery[] = $product->get_image_id() ?: 0; }
            ...
            echo '<img id="mh-main-img" src="'.esc_url($main_large_url).'" data-full="'.esc_url($main_large_url).'" data-thumb="'.esc_url($main_thumb_url).'" data-role="main" alt="'.esc_attr(get_post_meta($main_id, '_wp_attachment_image_alt', true)).'" />';
          ?>
        </div>
      ...
      <div class="product-details">
        ...
        <div class="product-actions">
          <form class="variations_form cart" method="post">
            ...
            <button type="submit" class="buy-button" id="mh-buy-button" disabled>BUY THIS SHIT!!!</button>
          </form>
        </div>
      </div>
    </div>
...
jQuery(function($){
  ...
  $(document).on('click', '.mh-thumb', function(){
    ...
    var thumbSrc=$thumb.attr('src'), thumbFull=$thumb.data('full'), thumbRole=$thumb.data('role');
    var mainSrc=$main.attr('src'), mainFull=$main.data('full'), mainThumb=$main.data('thumb'), mainRole=$main.data('role');
    $main.attr('src',thumbFull).data('full',thumbFull).data('thumb',thumbSrc).data('role',thumbRole).removeAttr('srcset');
    $thumb.attr('src',mainThumb).data('full',mainFull).data('thumb',mainThumb).data('role',mainRole);
  });
  function toggleBuy(){
    var ok=$('.size-btn.selected').length>0&&!$('.size-btn.selected').hasClass('disabled');
    $('#mh-buy-button').prop('disabled',!ok);
    if(ok) $('#mh-quantity').prop('disabled',false);
  }
